...
print("CRON RUN START", file=sys.stderr)
...
def main():
    ...
    print(f"Checking {len(urls)} feeds for updates since {cutoff_date.strftime('%Y-%m-%d')}...", file=sys.stderr)
    ...
    output_lines = []
    if not all_updates:
        output_lines.append(f"Aucune mise à jour trouvée dans les dernières {args.days} jour(s).")
    else:
        output_lines.append(f"Found {len(all_updates)} updates:\n")
        for item in all_updates:
            output_lines.append(f"### {item['title']}")
            output_lines.append(f"- **Source**: {item['feed_title']}")
            output_lines.append(f"- **Author**: {item['author']}")
            output_lines.append(f"- **Date**: {item['updated'].strftime('%Y-%m-%d %H:%M')}")
